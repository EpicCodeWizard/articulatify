...
def fixcase(mystr):
    if mystr == mystr.upper() or " AND " in mystr or " OR " in mystr: return titlecase(mystr)
    return mystr


def processreveiving(course):
    finval = ""
...
def processarts(out):
    ...
    for course in out:
        ...
        finval = processreceiving(course)
        if len(finval) == 0:
    ...
    for key, value in sir.items():
        lst = [x for x in list(set(value)) if "REQUIREMENT: " not in x and "TRANSFERABLE" not in x]
...
def process_articulation(source_school, destination_schools, year_range, include_classname, include_destcoursename):
    ...
    yield generate_spreadsheet(log, myfin, ucclassnames, classnames, source_school)


def generate_spreadsheet(log, myfin, ucclassnames, classnames, source_school):
...
def clearyear(): return gr.Dropdown(label="Year", visible=False)

def get_destination_schools(source_school):
    if len(source_school) == 0: return gr.Dropdown(label="Destination Institutions", multiselect=True, visible=False)
...
def get_years(source_school, destination_schools):
    if len(destination_schools) == 0: return gr.Dropdown(label="Year", visible=False)
    global rawyears
    lolfin = []
